...
Via NuGet:
    ...
    dotnet add package Kusumi512-Standalone --version 1.0.2
...
- **Do not log or transmit keys, nonces, or plaintext.** Logging or printing key/nonce/plaintext values—even in debug builds—can expose them in log aggregators, crash reporters, or monitoring dashboards. Production code **must not** emit sensitive material to any output stream.
...
This behaviour is unconditional—it applies regardless of whether encryption or decryption was ever performed. Use a `using` statement or `try/finally` block to ensure timely disposal:
